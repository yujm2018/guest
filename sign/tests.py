...
class TestModels(TestCase):
    def setUp(self):
        ...
        Guest.objects.create(id=1, event_id=12, realname='alen', phone='13711001101',email='alen@mail.com', sign=False)

    def test_event(self):
        event=Event.objects.get(id=1)
        self.assertEquals(event.name,"oneplus 3 event")
        self.assertEquals(event.address, "shenzhen")

    def test_guest(self):
        guest = Guest.objects.get(id=1)
        self.assertEquals(guest.realname, "alen")
        self.assertEquals(guest.email, "alen@mail.com")
...
class IndexPageTest(TestCase):
    ...
    def test_index_page_renders_index_template(self):
        ...
        self.assertTemplateUsed(response, 'index_bs.html')
